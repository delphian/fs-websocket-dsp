 ...
 * @details Parse binary stream of processing request into fs_ws_signal_request structure.
 ...
 * @param[in] response Signal processing response.
 */
char *fs_ws_signal_serialize_response(struct fs_ws_signal_response);

/**
 * @brief Calculate size_t required to malloc a serialized form of signal processing response.
 * @param[in] response Signal processing response.
 */
size_t fs_ws_signal_get_response_size(struct fs_ws_signal_response);
